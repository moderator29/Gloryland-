 ...
 */
function accruedAt(principal: number, openedAt: number, now: number): number {
  const days = clamp((now - openedAt) / DAY_MS, 0, CYCLE_DAYS);
...
export function derive(events: LedgerEvent[], now: number = Date.now()): Snapshot {
  ...
  const closedIds = new Set(closes.map((c) => c.positionId));

  const positions: Position[] = opens.map((o) => {
    const tier = tierById(o.tierId) ?? TIERS[0];
    const daysElapsed = clamp((now - o.at) / DAY_MS, 0, CYCLE_DAYS);
    const accrued = accruedAt(o.amount, o.at, now);

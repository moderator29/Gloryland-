...
import { DAY_RATE, entryLabel } from "./figures";

/**
 * The numbers, worked through live.
 *
 * Not an illustration of a product: it is the product's own arithmetic, run
 * against a principal and a run of days the visitor picks. Every figure comes
 * from `domain/tiers`, so this panel cannot drift away from what the portal
 * will actually compute, and the formula is printed above the result so the
 * numbers can be checked by hand rather than taken on trust.
 *
 * A day control exists because there is no term any more. The old version had
 * one answer, the value at maturity; this one has as many answers as there are
 * days, and letting the visitor pick the day is the only honest way to show
 * that. Nothing here compounds: it is principal times rate times days.
 */

/** Entry thresholds, thinned to eight so the control fits at 360px. */
const SIZES = TIERS.filter((_, i) => i % 3 === 0 || i === TIERS.length - 1).map((t) => t.entry);

/** Day counts worth asking about. The first is the withdrawal window. */
const SPANS = [WITHDRAW_INTERVAL_DAYS, 10, 30, 90];
  ...
  return (
    <div className="inset p-4 sm:p-5">
      ...
      <div
        ...
        className="mt-4 grid grid-cols-4 gap-1.5 sm:grid-cols-8"
      >
        {SIZES.map((amount) => {
      ...
      <div role="group" aria-label="Days held" className="mt-2 grid grid-cols-4 gap-1.5">
        {SPANS.map((n) => {
          const on = n === days;
          return (
            <button
      ...
      <dl className="mt-4 grid grid-cols-2 gap-x-5 gap-y-4 lg:grid-cols-4">
        ...
        <Figure label={`Reward, ${days} days`} value={money(reward)} tone="gain" />
        <div className="min-w-0">
          <dt className="tag-micro block min-h-[2.2em]">Principal and reward</dt>
      ...
      <p className="mt-4 border-t border-[var(--line)] pt-3 text-[11px] leading-relaxed text-[var(--text-low)]">
        Arithmetic from the published rate, applied to a principal and a run of days you chose. It
        is not a forecast, not an offer, and not a guarantee that the amount will be paid.
      </p>
    </div>
  );
}
...
function Figure({
  ...
  return (
    <div className="min-w-0">
      {/* Reserved height, because "Reward, 30 days" wraps to two lines while
